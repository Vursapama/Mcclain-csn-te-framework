...
def adaptive_te_coefficients(delta: np.ndarray, memory_gain: float, memory_decay: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    # === HARMONIC MULTIPLIER ===
    # Doubling Family Tension Weight to anchor the 0.002 precision
...
def compute_delta_conditioned_estimator(
    ...
    # Kernel window logic restored to prevent singular matrices
    ...
    for i in range(len(panel_rows)):
        ...
        if mode == "opt_local":
            ...
            # === STAGE 11: VULNERABLE PRECISION (v21) ===
            # Boundary 1.15, Sharpened Knee Width 0.35
            # Center = Unleashed Specialist (0.002), Edge = Protective Guardian (0.85)
            transition = 1.0 / (1.0 + np.exp((abs(delta[i]) - 1.15) / 0.35))
            l_reg = 0.002 * transition + 0.85 * (1.0 - transition)
            
            # Regression across the full k-weighted window matrix
...
def make_plots(panel: List[Dict[str, float]], memory_rows: List[Dict[str, float]], optimal_rows: List[Dict[str, float]], out_dir: str) -> List[str]:
    ...
    ax.legend()
    p = os.path.join(out_dir, "stat_v21_figure1_estimators_vs_reference.png")
    ...
    ax.grid(True, alpha=0.3)
    ax.legend()
    p = os.path.join(out_dir, "stat_v21_figure2_signed_errors.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    p = os.path.join(out_dir, "stat_v21_figure3_memory_surfaces_slices.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    p = os.path.join(out_dir, "stat_v21_figure4_resonance_concentration.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    for ax in axes.flat:
        ...
        ax.grid(True, alpha=0.3)
    axes[0, 0].legend(fontsize=8)
    p = os.path.join(out_dir, "stat_v21_figure5_optimal_estimator_weights.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    return out
...
def weight_surface_feature_matrix(delta: np.ndarray, memory_gain: np.ndarray, memory_decay: np.ndarray) -> np.ndarray:
    ...
    pulse = 1.0 / (1.0 + np.exp((np.abs(delta) - 1.15) / 0.35))
...
def make_predictive_surface_plots(panel_rows: List[Dict[str, float]], coeffs: Dict[str, Dict[str, np.ndarray]], predict_rows: List[Dict[str, float]], out_dir: str) -> List[str]:
    ...
    axes[1].legend()
    p = os.path.join(out_dir, "stat_v21_figure6_predictive_surface_baseline.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    for ax in axes:
        ...
        ax.legend()
        ax.set_xlabel("detuning")
    p = os.path.join(out_dir, "stat_v21_figure7_predictive_surface_weights.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    p = os.path.join(out_dir, "stat_v21_figure8_predictive_surface_memory.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    for ax,title in zip(axes.ravel(),titles):
        ...
        ax.legend(fontsize=8)
        ax.set_xlabel("memory_gain")
    p = os.path.join(out_dir, "stat_v21_figure9_te_managed_objective.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
...
def make_robustness_plots(robust_rows: List[Dict[str, float]], out_dir: str) -> List[str]:
    ...
    p = os.path.join(out_dir, "stat_v21_figure10_robustness.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    for ax, name in zip(axes, ["phase", "centroid", "peak"]):
        ...
        ax.grid(True, alpha=0.3)
        ax.legend(fontsize=7)
    p = os.path.join(out_dir, "stat_v21_figure11_robust_weights.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
...
def main() -> None:
    print("[start] csn_te_stats_v21_robustness (Vulnerable Precision)", flush=True)
    base = SimParams(
        gamma=1.0,
        alpha=1.5,
        delta0=0.0,
    ...
    out_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "csn_te_stats_v21_robustness_outputs")
    os.makedirs(out_dir, exist_ok=True)
