...
# ============================================================
# SECTION 1. GENERATIVE SYSTEM (THE MEDIUM)
# ============================================================
...
def adaptive_te_coefficients(delta: np.ndarray, memory_gain: float, memory_decay: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    # === STAGE 16: THE MASTER FADER ===
    # 2.0x Consensus Penalty Restored to enforce absolute harmonic agreement 
    # while the gain vector scales the amplitude.
    beta = 2.0 * (0.18 + 0.42 * tail + 0.08 * memory_ratio)
...
# === STAGE 16: GAIN COMPENSATOR (UNCONSTRAINED TIKHONOV) ===
def solve_tikhonov_gain_compensated(X: np.ndarray, y: np.ndarray, lambda_reg: float = 0.05) -> np.ndarray:
    n = X.shape[1]
    X_std = np.std(X, axis=0) + 1e-9
    X_scaled = X / X_std 
    ...
    try:
        w_scaled = np.linalg.solve(G, c)
    except np.linalg.LinAlgError:
        w_scaled = np.linalg.lstsq(G, c, rcond=None)[0]
        
    return w_scaled / X_std


# === REGIME SPLIT HELPER ===
def split_core_tail(delta_values, threshold=1.25):
...
def compute_delta_conditioned_estimator(
    ...
    for i in range(len(panel_rows)):
        ...
        if mode == "opt_local":
            ...
            # === STAGE 16: GAIN COMPENSATOR ===
            # Boundary 1.25, Knee Width 0.50 (V24 Chassis)
            # NO Sum-to-One constraint applied here.
            transition = 1.0 / (1.0 + np.exp((abs(delta[i]) - 1.25) / 0.50))
            l_reg = 0.0001 * transition + 1.0 * (1.0 - transition)
            
            w = solve_tikhonov_gain_compensated(X * np.sqrt(k)[:, None], ref * np.sqrt(k), lambda_reg=l_reg)
            # Intentionally NOT projecting sum to 1 to allow amplitude compensation
        else:
        ...
        if mode in ["opt_local_sum1"]:
            w_state = project_sum1(w_state)
        elif mode == "opt_local_sum1_nn":
            w_state = project_to_simplex(w_state)
        # Note: opt_local_tikhonov remains unprojected for gain tracking
...
def compute_optimal_estimator_for_panel(panel_rows: List[Dict[str, float]]) -> Tuple[Dict[str, float], List[Dict[str, float]], Dict[str, Dict[str, float]]]:
    ...
    weights_tikhonov = solve_tikhonov_gain_compensated(X, ref, lambda_reg=0.05)
...
def make_plots(panel: List[Dict[str, float]], memory_rows: List[Dict[str, float]], optimal_rows: List[Dict[str, float]], out_dir: str) -> List[str]:
    ...
    ax.legend()
    p = os.path.join(out_dir, "stat_v26_figure1_estimators_vs_reference.png")
    ...
    ax.grid(True, alpha=0.3)
    ax.legend()
    p = os.path.join(out_dir, "stat_v26_figure2_signed_errors.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    p = os.path.join(out_dir, "stat_v26_figure3_memory_surfaces_slices.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    p = os.path.join(out_dir, "stat_v26_figure4_resonance_concentration.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    for ax in axes.flat:
        ...
        ax.grid(True, alpha=0.3)
    axes[0, 0].legend(fontsize=8)
    p = os.path.join(out_dir, "stat_v26_figure5_optimal_estimator_weights.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    return out
...
def weight_surface_feature_matrix(delta: np.ndarray, memory_gain: np.ndarray, memory_decay: np.ndarray) -> np.ndarray:
    ...
    pulse = 1.0 / (1.0 + np.exp((np.abs(delta) - 1.25) / 0.50))
...
def fit_predictive_weight_surface(training_rows: List[Dict[str, float]]) -> Dict[str, Dict[str, np.ndarray]]:
    ...
    core_mask, tail_mask = split_core_tail(delta, threshold=1.25)
...
def predict_weight_surface(delta: np.ndarray, memory_gain: float, memory_decay: float, coeffs: Dict[str, Dict[str, np.ndarray]]) -> np.ndarray:
    ...
    for mask, regime in [(core_mask, "core"), (tail_mask, "tail")]:
        ...
        # STAGE 16: Intentionally omitting sum-to-1 projection to preserve gain compensation.
        for i in range(w_regime.shape[0]):
            pass # w_regime[i] = project_sum1(w_regime[i])
        W[mask] = w_regime
...
def summarize_predictive_surface_by_memory(base: SimParams, coeffs: Dict[str, Dict[str, np.ndarray]], decays: List[float], gains: List[float], detunings: np.ndarray) -> List[Dict[str, float]]:
    ...
    for md in decays:
        ...
        for mg in gains:
            ...
            # Track weight sum to monitor unconstrained gain amplitude scaling
            weight_sum_mean = float(np.mean(np.sum(W, axis=1)))
            ...
            rows.append({
                ...
                "predict_weight_sum_mean": weight_sum_mean,
...
def make_predictive_surface_plots(panel_rows: List[Dict[str, float]], coeffs: Dict[str, Dict[str, np.ndarray]], predict_rows: List[Dict[str, float]], out_dir: str) -> List[str]:
    ...
    axes[1].legend()
    p = os.path.join(out_dir, "stat_v26_figure6_predictive_surface_baseline.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    for ax in axes:
        ...
        ax.legend()
        ax.set_xlabel("detuning")
    p = os.path.join(out_dir, "stat_v26_figure7_predictive_surface_weights.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    p = os.path.join(out_dir, "stat_v26_figure8_predictive_surface_memory.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    for ax,title in zip(axes.ravel(),titles):
        ...
        ax.legend(fontsize=8)
        ax.set_xlabel("memory_gain")
    p = os.path.join(out_dir, "stat_v26_figure9_te_managed_objective.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
...
def apply_predictive_surface_to_panel_with_weight_jitter(panel_rows: List[Dict[str, float]], coeffs: Dict[str, Dict[str, np.ndarray]],
    ...
    if weight_jitter_sigma > 0.0:
        ...
        # STAGE 16: No sum-to-one projection applied to jittered weights to allow amplitude variation
...
def make_robustness_plots(robust_rows: List[Dict[str, float]], out_dir: str) -> List[str]:
    ...
    p = os.path.join(out_dir, "stat_v26_figure10_robustness.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    for ax, name in zip(axes, ["phase", "centroid", "peak"]):
        ...
        ax.grid(True, alpha=0.3)
        ax.legend(fontsize=7)
    p = os.path.join(out_dir, "stat_v26_figure11_robust_weights.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
...
def simulate_temporal_shock_response(base: SimParams, coeffs: Dict[str, Dict[str, np.ndarray]],
    ...
    def predict_weights(delta0: float, mg: float, md: float):
        regime = "core" if abs(delta0) <= 1.25 else "tail"
        ...
        # STAGE 16: No sum-to-1 projection
...
def main() -> None:
    print("[start] Stage 16: The Gain Compensator (v26)", flush=True)
    out_dir = os.path.join(os.getcwd(), "csn_te_stats_v26_outputs")
    os.makedirs(out_dir, exist_ok=True)
    ...
    # 1. Base Panel and Optimal Estimators
    ...
    print("\nPhase: The Gain Compensator Build Complete. Verifying Bias Neutralization.")
