...
# === REGIME SPLIT HELPER ===
def split_core_tail(delta_values, threshold=1.0):
...
def compute_delta_conditioned_estimator(
    ...
    for i in range(len(panel_rows)):
        ...
        if mode == "opt_local":
            ...
            # === STAGE 6: THE QUANTIZED PULSE (CSN ENTROPY FLOOR) ===
            # Sharp Sigmoid Transition representing the Nyquist Limit
            # Core lambda = 0.01 (I AM / High Torque), Tail lambda = 1.0 (Not-Self / Damped)
            transition = 1.0 / (1.0 + np.exp((abs(delta[i]) - 0.75) / 0.15))
            l_reg = 0.01 * transition + 1.0 * (1.0 - transition)
...
def make_plots(panel: List[Dict[str, float]], memory_rows: List[Dict[str, float]], optimal_rows: List[Dict[str, float]], out_dir: str) -> List[str]:
    ...
    ax.legend()
    p = os.path.join(out_dir, "stat_v1_figure1_estimators_vs_reference.png")
    ...
    ax.grid(True, alpha=0.3)
    ax.legend()
    p = os.path.join(out_dir, "stat_v1_figure2_signed_errors.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    p = os.path.join(out_dir, "stat_v1_figure3_memory_surfaces_slices.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    p = os.path.join(out_dir, "stat_v1_figure4_resonance_concentration.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    for ax in axes.flat:
        ...
        ax.grid(True, alpha=0.3)
    axes[0, 0].legend(fontsize=8)
    p = os.path.join(out_dir, "stat_v1_figure5_optimal_estimator_weights.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    return out
...
def weight_surface_feature_matrix(delta: np.ndarray, memory_gain: np.ndarray, memory_decay: np.ndarray) -> np.ndarray:
    ...
    pulse = 1.0 / (1.0 + np.exp((np.abs(delta) - 0.75) / 0.15))
...
def make_predictive_surface_plots(panel_rows: List[Dict[str, float]], coeffs: Dict[str, Dict[str, np.ndarray]], predict_rows: List[Dict[str, float]], out_dir: str) -> List[str]:
    ...
    axes[1].legend()
    p = os.path.join(out_dir, "stat_v4_figure6_predictive_surface_baseline.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    for ax in axes:
        ...
        ax.legend()
        ax.set_xlabel("detuning")
    p = os.path.join(out_dir, "stat_v4_figure7_predictive_surface_weights.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    p = os.path.join(out_dir, "stat_v4_figure8_predictive_surface_memory.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    for ax,title in zip(axes.ravel(),titles):
        ...
        ax.legend(fontsize=8)
        ax.set_xlabel("memory_gain")
    p = os.path.join(out_dir, "stat_v4_figure9_te_managed_objective.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
...
def make_robustness_plots(robust_rows: List[Dict[str, float]], out_dir: str) -> List[str]:
    ...
    p = os.path.join(out_dir, "stat_v6_figure10_robustness.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    for ax, name in zip(axes, ["phase", "centroid", "peak"]):
        ...
        ax.grid(True, alpha=0.3)
        ax.legend(fontsize=7)
    p = os.path.join(out_dir, "stat_v6_figure11_robust_weights.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
...
def main() -> None:
    print("[start] csn_te_stats_v6_robustness (The Quantized Pulse)", flush=True)
    base = SimParams(
        gamma=1.0,
        alpha=1.5,
        delta0=0.0,
    ...
    out_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "csn_te_stats_v6_robustness_outputs")
    os.makedirs(out_dir, exist_ok=True)
