...
def compute_delta_conditioned_estimator(
    ...
    for i in range(len(panel_rows)):
        ...
        if mode == "opt_local":
            ...
            # === STAGE 7: THE HIGH-TORQUE PULSE (SOFT-KNEE CROSSOVER) ===
            # Boundary shifted to 1.05, Knee Width expanded to 0.45
            # Center = Unleashed Specialist (0.005), Edge = Protective Guardian (0.80)
            transition = 1.0 / (1.0 + np.exp((abs(delta[i]) - 1.05) / 0.45))
            l_reg = 0.005 * transition + 0.80 * (1.0 - transition)
...
def weight_surface_feature_matrix(delta: np.ndarray, memory_gain: np.ndarray, memory_decay: np.ndarray) -> np.ndarray:
    ...
    pulse = 1.0 / (1.0 + np.exp((np.abs(delta) - 1.05) / 0.45))
...
def main() -> None:
    print("[start] csn_te_stats_v7_robustness (The High-Torque Pulse)", flush=True)
    base = SimParams(
        gamma=1.0,
        alpha=1.5,
        delta0=0.0,
    ...
    out_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "csn_te_stats_v7_robustness_outputs")
    os.makedirs(out_dir, exist_ok=True)
