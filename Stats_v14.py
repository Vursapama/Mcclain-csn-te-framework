...
def compute_delta_conditioned_estimator(
    ...
    for i in range(len(panel_rows)):
        ...
        if mode == "opt_local":
            ...
            # === SHARPENED HORIZON: CONTINUOUS METAPHYSICAL GRADIENT ===
            # Width = 0.40, Core Floor = 0.05, Tail Damping = 0.75
            core_presence = np.exp(-0.5 * (delta[i] / 0.40)**2)
            l_reg = 0.05 * core_presence + 0.75 * (1.0 - core_presence)
