...
# === TIKHONOV UNCONSTRAINED SOLVER (STAGE 8) ===
def solve_tikhonov_unconstrained(X: np.ndarray, y: np.ndarray, lambda_reg: float = 0.05) -> np.ndarray:
    n = X.shape[1]
...
def compute_delta_conditioned_estimator(
    ...
    for i in range(len(panel_rows)):
        ...
        if mode == "opt_local":
            ...
            # === STAGE 8: THE UNLEASHED SPECIALIST (UNCONSTRAINED DRIFT) ===
            # Boundary 1.05, Knee Width sharpened to 0.30
            # Center = Unleashed Specialist (0.0001), Edge = Protective Guardian (0.85)
            # Weights are allowed to drift to absorb entropy floor
            transition = 1.0 / (1.0 + np.exp((abs(delta[i]) - 1.05) / 0.30))
            l_reg = 0.0001 * transition + 0.85 * (1.0 - transition)
            
            w = solve_tikhonov_unconstrained(X * np.sqrt(k)[:, None], ref * np.sqrt(k), lambda_reg=l_reg)
        else:
...
def compute_optimal_estimator_for_panel(panel_rows: List[Dict[str, float]]) -> Tuple[Dict[str, float], List[Dict[str, float]], Dict[str, Dict[str, float]]]:
    ...
    weights_tikhonov = solve_tikhonov_unconstrained(X, ref, lambda_reg=0.05)
...
def make_plots(panel: List[Dict[str, float]], memory_rows: List[Dict[str, float]], optimal_rows: List[Dict[str, float]], out_dir: str) -> List[str]:
    ...
    ax.legend()
    p = os.path.join(out_dir, "stat_v18_figure1_estimators_vs_reference.png")
    ...
    ax.grid(True, alpha=0.3)
    ax.legend()
    p = os.path.join(out_dir, "stat_v18_figure2_signed_errors.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    p = os.path.join(out_dir, "stat_v18_figure3_memory_surfaces_slices.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    p = os.path.join(out_dir, "stat_v18_figure4_resonance_concentration.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    for ax in axes.flat:
        ...
        ax.grid(True, alpha=0.3)
    axes[0, 0].legend(fontsize=8)
    p = os.path.join(out_dir, "stat_v18_figure5_optimal_estimator_weights.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    return out
...
def predict_weight_surface(delta: np.ndarray, memory_gain: float, memory_decay: float, coeffs: Dict[str, Dict[str, np.ndarray]]) -> np.ndarray:
    ...
    for mask, regime in [(core_mask, "core"), (tail_mask, "tail")]:
        ...
        # Unconstrained Tikhonov: NO sum-to-1 projection applied to the weights
        W[mask] = w_regime
...
def make_predictive_surface_plots(panel_rows: List[Dict[str, float]], coeffs: Dict[str, Dict[str, np.ndarray]], predict_rows: List[Dict[str, float]], out_dir: str) -> List[str]:
    ...
    axes[1].legend()
    p = os.path.join(out_dir, "stat_v18_figure6_predictive_surface_baseline.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    for ax in axes:
        ...
        ax.legend()
        ax.set_xlabel("detuning")
    p = os.path.join(out_dir, "stat_v18_figure7_predictive_surface_weights.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    p = os.path.join(out_dir, "stat_v18_figure8_predictive_surface_memory.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    for ax,title in zip(axes.ravel(),titles):
        ...
        ax.legend(fontsize=8)
        ax.set_xlabel("memory_gain")
    p = os.path.join(out_dir, "stat_v18_figure9_te_managed_objective.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
...
def apply_predictive_surface_to_panel_with_weight_jitter(panel_rows: List[Dict[str, float]], coeffs: Dict[str, Dict[str, np.ndarray]],
    ...
    if weight_jitter_sigma > 0.0:
        ...
        # Unconstrained Tikhonov: NO sum-to-1 projection applied to the jittered weights
...
def make_robustness_plots(robust_rows: List[Dict[str, float]], out_dir: str) -> List[str]:
    ...
    p = os.path.join(out_dir, "stat_v18_figure10_robustness.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    for ax, name in zip(axes, ["phase", "centroid", "peak"]):
        ...
        ax.grid(True, alpha=0.3)
        ax.legend(fontsize=7)
    p = os.path.join(out_dir, "stat_v18_figure11_robust_weights.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
...
def simulate_temporal_shock_response(base: SimParams, coeffs: Dict[str, Dict[str, np.ndarray]],
    ...
    def predict_weights(delta0: float, mg: float, md: float):
        ...
        # Unconstrained Tikhonov: NO sum-to-1 projection
        ...
        for mg in gains:
            for delta_base in deltas:
                ...
                for t in range(total_steps):
                    ...
                    # Unconstrained Tikhonov: Just add the jitter, no projection
                    w = w + jitter
...
def main() -> None:
    print("[start] csn_te_stats_v18_robustness (The Unleashed Specialist - Unconstrained Drift)", flush=True)
    base = SimParams(
        gamma=1.0,
        alpha=1.5,
        delta0=0.0,
    ...
    out_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "csn_te_stats_v18_robustness_outputs")
    os.makedirs(out_dir, exist_ok=True)
