...
def adaptive_te_coefficients(delta: np.ndarray, memory_gain: float, memory_decay: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    # === STAGE 12: CRITICAL DAMPING ===
    # Reverting Family Tension Weight to 1.0x to allow specialist divergence
    beta = 1.0 * (0.18 + 0.42 * tail + 0.08 * memory_ratio)
...
def compute_delta_conditioned_estimator(
    ...
    for i in range(len(panel_rows)):
        ...
        if mode == "opt_local":
            ...
            # === STAGE 12: CRITICAL DAMPING (v22) ===
            # Boundary 1.05, Sharpened Knee Width 0.35
            # Center = Unleashed Specialist (0.002), Edge = Protective Guardian (0.85)
            transition = 1.0 / (1.0 + np.exp((abs(delta[i]) - 1.05) / 0.35))
...
def make_plots(panel: List[Dict[str, float]], memory_rows: List[Dict[str, float]], optimal_rows: List[Dict[str, float]], out_dir: str) -> List[str]:
    ...
    ax.legend()
    p = os.path.join(out_dir, "stat_v22_figure1_estimators_vs_reference.png")
    ...
    ax.grid(True, alpha=0.3)
    ax.legend()
    p = os.path.join(out_dir, "stat_v22_figure2_signed_errors.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    p = os.path.join(out_dir, "stat_v22_figure3_memory_surfaces_slices.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    p = os.path.join(out_dir, "stat_v22_figure4_resonance_concentration.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    for ax in axes.flat:
        ...
        ax.grid(True, alpha=0.3)
    axes[0, 0].legend(fontsize=8)
    p = os.path.join(out_dir, "stat_v22_figure5_optimal_estimator_weights.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    return out
...
def weight_surface_feature_matrix(delta: np.ndarray, memory_gain: np.ndarray, memory_decay: np.ndarray) -> np.ndarray:
    ...
    pulse = 1.0 / (1.0 + np.exp((np.abs(delta) - 1.05) / 0.35))
...
def predict_weight_surface(delta: np.ndarray, memory_gain: float, memory_decay: float, coeffs: Dict[str, Dict[str, np.ndarray]]) -> np.ndarray:
    ...
    for mask, regime in [(core_mask, "core"), (tail_mask, "tail")]:
        if not np.any(mask): continue
        # Properly call the full 15-variable feature matrix
...
def make_predictive_surface_plots(panel_rows: List[Dict[str, float]], coeffs: Dict[str, Dict[str, np.ndarray]], predict_rows: List[Dict[str, float]], out_dir: str) -> List[str]:
    ...
    axes[1].legend()
    p = os.path.join(out_dir, "stat_v22_figure6_predictive_surface_baseline.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    for ax in axes:
        ...
        ax.legend()
        ax.set_xlabel("detuning")
    p = os.path.join(out_dir, "stat_v22_figure7_predictive_surface_weights.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    p = os.path.join(out_dir, "stat_v22_figure8_predictive_surface_memory.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    for ax,title in zip(axes.ravel(),titles):
        ...
        ax.legend(fontsize=8)
        ax.set_xlabel("memory_gain")
    p = os.path.join(out_dir, "stat_v22_figure9_te_managed_objective.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
...
def make_robustness_plots(robust_rows: List[Dict[str, float]], out_dir: str) -> List[str]:
    ...
    p = os.path.join(out_dir, "stat_v22_figure10_robustness.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
    ...
    for ax, name in zip(axes, ["phase", "centroid", "peak"]):
        ...
        ax.grid(True, alpha=0.3)
        ax.legend(fontsize=7)
    p = os.path.join(out_dir, "stat_v22_figure11_robust_weights.png")
    fig.savefig(p, dpi=150)
    plt.close(fig)
    out.append(p)
...
def simulate_temporal_shock_response(base: SimParams, coeffs: Dict[str, Dict[str, np.ndarray]],
    ...
    def predict_weights(delta0: float, mg: float, md: float):
        ...
        # Access the properly dimensioned global matrix
...
def main() -> None:
    print("[start] csn_te_stats_v22_robustness (Critical Damping)", flush=True)
    out_dir = os.path.join(os.getcwd(), "csn_te_stats_v22_outputs")
    os.makedirs(out_dir, exist_ok=True)
    ...
    print("\nPhase: Critical Damping Build Complete. Checking Tension Anchor Release.")
