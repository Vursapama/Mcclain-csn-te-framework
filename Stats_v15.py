...
def compute_delta_conditioned_estimator(
    ...
    for i in range(len(panel_rows)):
        ...
        if mode == "opt_local":
            ...
            # === THE BALANCED ANCHOR: CONTINUOUS METAPHYSICAL GRADIENT ===
            # Width restored to 1.0, Core Floor = 0.05, Tail Damping softened to 0.50
            core_presence = np.exp(-0.5 * (delta[i] / 1.0)**2)
            l_reg = 0.05 * core_presence + 0.50 * (1.0 - core_presence)
